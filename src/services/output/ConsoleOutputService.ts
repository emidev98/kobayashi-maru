...
export default class ConsoleOutputService implements OutputService {
	...
	sendDeposits(deposits: Array<Deposit>): Promise<boolean> {
		_.forEach(deposits, (deposit) => {
			...
			if (deposit.customer) {
				depositOutput = `Deposited for ${deposit.customer.name}: count=${
					deposit.totalTransactions
				} sum=${deposit.walletBalance.toFixed(8)}`;
			} else {
				depositOutput = `Deposited without reference: count=${
					deposit.totalTransactions
				} sum=${deposit.walletBalance.toFixed(8)}`;
			}
