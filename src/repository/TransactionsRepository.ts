...
export default class TransactionsRepository extends MongoDBRepository {
	...
	getSmallestTransactionFromRepository = async () => {
		const collection = (await this.connect()).collection(this.COLLECTION);
	...
	insertTransaction = async (transactions: Array<Transaction>) => {
		return (await this.connect()).collection(this.COLLECTION).insertMany(transactions);
	};

	updateTransaction = async (transaction: Transaction) => {
		return (await this.connect()).collection(this.COLLECTION).updateOne(
			{
				txid: {
					$eq: transaction._id
				}
