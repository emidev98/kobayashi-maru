...
export default class BlockchainsController {
	...
	async storeTransactions(transactions: Array<Transaction>) {
		...
		if (!_.isEmpty(transactionsToUpdate)) {
			...
			transactionsToUpdate = _.differenceWith(
				transactions,
				transactionsToUpdate,
				(transaction, transactionToUpdate) => {
					return (
						transaction.confirmations >= transactionToUpdate.confirmations &&
			...
			const transactionsAfterUpdate = await Promise.all(
				_.map(transactionsToUpdate, async (transaction) => {
