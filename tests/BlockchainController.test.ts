...
describe("BlockchainsController", () => {
	it("Get transactions from blockchain filter by confirmations and order by timestamp", async () => {
        ...
        // When
        transactions = await blockchainsController.getTransactionsFromChain();

        // Then
        expect(202).toBe(transactions.length);
        expect(
            transactions.map(transaction => transaction.timereceived)
        ).toEqual(
	...
	it("Store parsed transactions on repository", async () => {
        // Given
        let transactions = await blockchainsController.getTransactionsFromChain();
